"""
----------------
Note by R01K: This module has been slightly modified (compare to
original env\Lib\site-packages\wmi.py) to support DataConduIT.
----------------
...
class SelfDeprecatingDict(object):
    ...
    def __getattr__(self, attribute):
        if attribute in self.dict_only:
            warnings.warn("In future this will be a list and not a dictionary",
                          DeprecationWarning)
...
def handle_com_error(err=None):
    ...
    if additional_info:
        ...
        exception_string.append(
            "  %s - %s" % (hex(scode), (error_description or "").strip()))
...
def from_time(year=None, month=None, day=None, hours=None, minutes=None, seconds=None,
              microseconds=None, timezone=None):
    ...
    def __init__(self, ole_object, method_name):
        ...
        try:
            ...
            if self.in_parameters is None:
                ...
                self.in_parameter_names = [(i.Name, i.IsArray) for i in
                                           self.in_parameters.Properties_]
            if self.out_parameters is None:
                self.out_parameter_names = []
            else:
                self.out_parameter_names = [(i.Name, i.IsArray) for i in
                                            self.out_parameters.Properties_]

            doc = "%s (%s) => (%s)" % (
                method_name,
                ", ".join([name + ("", "[]")[is_array] for (name, is_array) in
                           self.in_parameter_names]),
                ", ".join([name + ("", "[]")[is_array] for (name, is_array) in
                           self.out_parameter_names])
            )
                ...
                for k, v in kwargs.items():
                    ...
                    if is_array is None:
                        raise AttributeError(
                            "%s is not a valid parameter for %s" % (k, self.__doc__))
                    else:
                        if is_array:
                            try:
                                list(v)
...
class _wmi_object:
    ...
    def __init__(self, ole_object, instance_of=None, fields=[], property_map={}):
        try:
            ...
            _set(self, "qualifiers",
                 dict((q.Name, q.Value) for q in self.ole_object.Qualifiers_))
    ...
    def __repr__(self):
        ...
        try:
            return "<%s: %s>" % (
            self.__class__.__name__, self.Path_.Path.encode("ascii", "backslashreplace"))
        except pywintypes.com_error:
            handle_com_error()

    def _cached_properties(self, attribute):
        if self.properties[attribute] is None:
            self.properties[attribute] = _wmi_property(
                self.ole_object.Properties_(attribute))
    ...
    def __getattr__(self, attribute):
        ...
        try:
            if attribute in self.properties:
                ...
                factory = self.property_map.get(attribute,
                                                self.property_map.get(property.type,
                                                                      lambda x: x))
    ...
    def references(self, wmi_class=""):
        ...
        #
        try:
            return [_wmi_object(i) for i in
                    self.ole_object.References_(strResultClass=wmi_class)]
        except pywintypes.com_error:
            handle_com_error()
...
class _wmi_event(_wmi_object):
    ...
    def __init__(self, event, event_info, fields=[]):
        ...
        if event_info:
            event_type = self.event_type_re.match(event_info.Path_.Class).group(
                1).lower()
...
class _wmi_class(_wmi_object):
    ...
    def query(self, fields=[], **where_clause):
        ...
        try:
            ...
            if where_clause:
                wql += " WHERE " + " AND ".join(
                    ["%s = %r" % (k, str(v)) for k, v in where_clause.items()])
    ...
    def watch_for(
        ...
        **where_clause
        ...
        if notification_type.lower() not in valid_notification_types:
            raise x_wmi("notification_type must be one of %s" % ", ".join(
...
class _wmi_namespace:
    ...
    class_re = re.compile("(?<=FROM )\S+", flags=re.IGNORECASE)
    ...
    def watch_for(
        ...
        **where_clause
        ...
        if raw_wql:
            try:
                wmi_class = re.search(_wmi_namespace.class_re, raw_wql).group(0)
            except AttributeError:
                pass
    ...
    def _cached_classes(self, class_name):
        ...
        if class_name not in self._classes_map:
            self._classes_map[class_name] = _wmi_class(self,
                                                       self._namespace.Get(class_name))
    ...
    try:
        try:
            if wmi:
                ...
                if not moniker.lower().startswith(PROTOCOL):
                ...
                if user:
                    if privileges or suffix:
                        raise x_wmi_authentication(
                            "You can't specify privileges or a suffix as well as a username")
                    elif computer in (None, '', '.'):
                        raise x_wmi_authentication(
                            "You can only specify user/password for a remote connection")
                    else:
                        obj = connect_server(
                            server=computer,
                            namespace=namespace,
...
def construct_moniker(
    ...
    security = []
    if impersonation_level: security.append(
        "impersonationLevel=%s" % impersonation_level)
    if authentication_level: security.append(
...
def connect_server(
    ...
    if impersonation_level:
        try:
            impersonation = getattr(obj._constants,
                                    "wbemImpersonationLevel%s" % impersonation_level.title())
        except AttributeError:
            raise x_wmi_authentication(
                "No such impersonation level: %s" % impersonation_level)
    else:
        impersonation = None

    if authentication_level:
        try:
            authentication = getattr(obj._constants,
                                     "wbemAuthenticationLevel%s" % authentication_level.title())
        except AttributeError:
            raise x_wmi_authentication(
                "No such impersonation level: %s" % impersonation_level)
    else:
...
def Registry(
    ...
    warnings.warn(
        "This function can be implemented using wmi.WMI (namespace='DEFAULT').StdRegProv",
        DeprecationWarning)
    if not moniker:
        moniker = construct_moniker(
            computer=computer,
